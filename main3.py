...
class GameState:
    ...
    def __init__(self):
        ...
        self.items_on_ground = [
            ...
            {"name": "Зілля енергії", "x": 300, "y": 250},
...
class PyGameWidget(QWidget):
    ...
    def update_game(self):
        # Рух персонажа
        speed = 5
        if Qt.Key_Left in self.keys_pressed and self.game_state.player_x > 20:
            self.game_state.player_x -= speed
            self.game_state.energy = max(0, self.game_state.energy - 0.1)
            self.energy_changed.emit(self.game_state.energy)
        if Qt.Key_Right in self.keys_pressed and self.game_state.player_x < 480:
            self.game_state.player_x += speed
            self.game_state.energy = max(0, self.game_state.energy - 0.1)
            self.energy_changed.emit(self.game_state.energy)
        if Qt.Key_Up in self.keys_pressed and self.game_state.player_y > 20:
            self.game_state.player_y -= speed
            self.game_state.energy = max(0, self.game_state.energy - 0.1)
            self.energy_changed.emit(self.game_state.energy)
        if Qt.Key_Down in self.keys_pressed and self.game_state.player_y < 580:
            self.game_state.player_y += speed
            self.game_state.energy = max(0, self.game_state.energy - 0.1)
            self.energy_changed.emit(self.game_state.energy)
        ...
        for item in self.game_state.items_on_ground[:]:
            if (abs(self.game_state.player_x - item["x"]) < 30 and 
                ...
                if "Зілля здоров'я" in item["name"]:
                    ...
                    self.game_state.energy = min(self.game_state.max_energy, self.game_state.energy + 30)
                    self.energy_changed.emit(self.game_state.energy)
...
class GameInterface(QMainWindow):
    def __init__(self):
        ...
        self.game_widget.energy_changed.connect(self.update_energy)
        ...
        energy_label = QLabel("Енергія:")
        self.energy_bar = StatBar(self.game_state.energy, self.game_state.max_energy, "#3498DB")
    ...
    def use_item(self):
        ...
        if "Зілля здоров'я" in item_name:
            ...
            self.game_state.energy = min(self.game_state.max_energy, self.game_state.energy + 40)
            self.update_energy(self.game_state.energy)
            self.game_state.inventory.remove(item_name)
